...
// The cinematic heritage brand panel (left on desktop). When an admin
// uploads a panel image (Admin → Settings → Auth page image) the panel is
// JUST the photo — no text, chips or effects. Otherwise it stays the
// dark-pine night scene with a rising sun + RTL Sorani identity, paired
// with the English form on the right.
function BrandPanel() {
  const imageSrc = useAuthImageSrc()

  if (imageSrc) {
    return (
      <aside className="relative hidden min-h-dvh overflow-hidden lg:block" dir="rtl">
        <img src={imageSrc} alt="" className="absolute inset-0 size-full object-cover" draggable="false" />
      </aside>
    )
  }
